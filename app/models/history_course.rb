class HistoryCourse < ActiveRecord::Base
  ...
  validates_format_of :idGroup, :with => /^([a-zA-Z ñÑ]{0,255})$/i , :message => "Solo acepta letras y espacios"
  validates_presence_of :idParish , :message => "La Parroquia no puede ser vacio"
  validates_format_of :idParish, :with => /^([a-zA-Z ñÑ]{0,255})$/i , :message => "Solo acepta letras y espacios"
